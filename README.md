...
- Tổ chức file: 
    - `data`: chứa dữ liệu thu thập đượcvà dữ liệu sau khi tiền xử lý
...
- Mức lương: Dựa vào mức lương, nhóm sẽ loại bỏ những giá trị mang tên là ` Thảo thuận`. Nhưng dữ liệu thô thường có dạng `8 - 15 triệu`. Do đó nhóm sẽ tách ra thành ` Mức lương thấp nhất`, `Mức lương cao nhất` và `Mức lương trung bình` để có thể sử dụng được.
- Thời gian thử việc: Nhóm sẽ tiền hành tách số và chuyể cột này thành kiển float.
